...
public class GrayImage {
    ...
    private static int[] normalizePixelValues(int[] pixels, double targetMin, double targetMax) {
        ...
        for (int i = 0; i < pixels.length; ++i) {
            doublePixels[i] = (double) pixels[i];
        }
     ...
     *
     * @return int[][] pixel array
     */
    private int[][] pixels2Dfrom1D() {
        int[][] result = new int[width][height];
        int index1D = 0;
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
    ...
    private long[] calcHistogram(int[] grayPixelValues) {
        ...
        // Look at each pixel, get its brightness value, and increment counter for that brightness.
    ...
    /**
     * Converts to JavaFX Image
     *
     * @return WritableImage representation
     */
    public WritableImage toFXImage() {
        ...
        if (bitDepth != 8) {
            ...
            double[] temp = new double[pixels1D.length];
            for (int i = 0; i < temp.length; ++i) {
                temp[i] = (double) pixels1D[i];
            }

            int[] scaledTemp = normalizePixelValues(temp, 0, 255);

            GrayImage scaledGrayImage = new GrayImage();
            scaledGrayImage.width = width;
            scaledGrayImage.height = height;
            scaledGrayImage.bitDepth = bitDepth;
            scaledGrayImage.pixels1D = scaledTemp;
            scaledGrayImage.pixels2D = scaledGrayImage.pixels2Dfrom1D();
    ...
    public GrayImage logTrans(double c) {
        ...
        for (int i = 0; i < origPixels.length; ++i) {
            double logTransGrayValue = c * Math.log((origPixels[i] / 255.0) + 1.0);
